...
export class CreateStore implements ICreateStore {
  ...
  get isPublishDisabled(): boolean {
    return (
      ...
      !this.createPost.categoryId ||
      this.creating ||
      this.createPost.images.filter(img => img.loading).length > 0
    );
  }
  ...
  uploadImage = async (id: string, path: string): Promise<void> => {
    ...
    axios
      ...
      .then(response => {
        ...
        image.loading = false;
        const index = this.createPost.images.findIndex(
          img => img.id === image.id
        );
        if (index !== -1) {
  ...
  removeImage = async (id: string): Promise<void> => {
    this.createPost.images = this.createPost.images.filter(
      img => img.id !== id
    );
  };
