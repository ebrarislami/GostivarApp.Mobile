...
export class ProfileStore implements IProfileStore {
  ...
  get isAllNotificationsEnabled(): boolean {
    const isThereAnythingDisabled = this.enabledNotifications.filter(
  ...
  async loadProfile(): Promise<void> {
    ...
    try {
      const response = await axios.get(
        "https://dev-gostivarapp.herokuapp.com/api/user/6"
      );
  ...
  toggleNotification(id: string): void {
    const notificationIndex = this.enabledNotifications.findIndex(
      not => not.id === id
    );
    if (notificationIndex !== -1) {
      this.enabledNotifications = [
        ...this.enabledNotifications.slice(0, notificationIndex),
        {
          ...this.enabledNotifications[notificationIndex],
          enabled: !this.enabledNotifications[notificationIndex].enabled
        },
        ...this.enabledNotifications.slice(notificationIndex + 1)
      ];
    }
  }
  ...
  update = async (): Promise<void> => {
    ...
    const enabledNotifications: PostCategory[] = toJS(
      this.enabledPostCategories
    );
